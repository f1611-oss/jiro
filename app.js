...
const DRAFT_KEY = "jiro-log-record-draft-v1";
const scoreFields = [
  ["soup", "スープ"],
  ["noodles", "麺"],
  ["pork", "豚"],
...
function saveDraft() {
  ...
  const draft = {
    ...
    genre: String(data.get("genre") || genres[0]),
    photo: document.querySelector("#photo-data")?.value || "",
    photoPath: document.querySelector("#photo-path")?.value || "",
    price: String(data.get("price") || ""),
    prefecture: String(data.get("prefecture") || ""),
    station: String(data.get("station") || ""),
    wait: String(data.get("wait") || ""),
    comment: String(data.get("comment") || ""),
    favorite: data.get("favorite") === "on",
    revisit: data.get("revisit") === "on",
    scores: currentScoresSafe(),
  };
  localStorage.setItem(DRAFT_KEY, JSON.stringify(draft));
}

function currentScoresSafe() {
  return Object.fromEntries(scoreFields.map(([key]) => {
    const input = document.querySelector(`#score-${key}`);
    return [key, Number(input?.value ?? 5)];
  }));
}
...
function addPage() {
  ...
  const defaults = record || state.addPrefill || loadDraft() || {
